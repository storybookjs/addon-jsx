...
function serializeItem(item, options, delimit = true) {
  ...
  if (isImmutable(item)) {
    ...
    let el = item;
    if (typeof item.type === 'function' && item.type.name === '') el = item.type(item.props);
    ...
    result = options.execFunctionInChildren
      ? serializeItem(item(), options, delimit)
      : options.useFunctionCode
...
function jsxToString(component, options) {
    ...
    componentData.props = Object.keys(component.props)
      ...
      .map(key => {
        ...
        if (typeof opts.keyValueOverride[key] === 'function') {
          ...
          value = serializeItem(component.props[key], Object.assign({}, opts, key));
        }
  ...
  if (component.props.children) {
    ...
    const customOpts = Object.assign({}, opts, { execFunctionInChildren: true });
    if (Array.isArray(component.props.children)) {
      ...
      componentData.children = serializeItem(component.props.children, customOpts, false);
    }
    return (
